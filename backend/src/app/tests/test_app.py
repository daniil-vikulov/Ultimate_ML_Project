...
import os
from backend.src.app.app import allowed_file, file_is_image, app, censor_image, classify_image, detect_image
...
class TestFileFunctions(unittest.TestCase):
    ...
    @patch("backend.src.app.app.Image.open")
    ...
    @patch("backend.src.app.app.logger.error")
    @patch("backend.src.app.app.Image.open")
...
class FlaskAppTestCase(unittest.TestCase):
    ...
    @patch('backend.src.app.app.allowed_file', return_value=True)
    @patch('backend.src.app.app.file_is_image', return_value=False)
    ...
    @patch('backend.src.app.app.allowed_file', return_value=True)
    @patch('backend.src.app.app.file_is_image', return_value=True)
    @patch('backend.src.app.app.process_image_file')
    ...
    @patch('backend.src.app.app.allowed_file', return_value=True)
    @patch('backend.src.app.app.file_is_image', return_value=True)
    @patch('backend.src.app.app.process_image_file', side_effect=Exception('Ошибка при обработке изображения'))
...
class TestCensorImage(unittest.TestCase):
    @patch('backend.src.app.app.detector.detect', return_value=[])
    @patch('backend.src.app.app.jsonify', return_value=MagicMock())
    ...
    @patch('backend.src.app.app.detector.detect', return_value=[{'class': 'NUDITY', 'score': 0.95}])
    @patch('backend.src.app.app.detector.censor')
    @patch('backend.src.app.app.jsonify', return_value=MagicMock())
...
class TestClassifyImage(unittest.TestCase):
    @patch('backend.src.app.app.classifier.classify', return_value={'uploads/test_image.jpg': {'safe': 0, 'unsafe': 0}})
    @patch('backend.src.app.app.jsonify', return_value=MagicMock())
    ...
    @patch('backend.src.app.app.classifier.classify', side_effect=Exception('Classification error'))
    @patch('backend.src.app.app.jsonify', return_value=MagicMock())
...
class TestDetectImage(unittest.TestCase):
    ...
    @patch('backend.src.app.app.detector.detect', side_effect=Exception('Detection error'))
    @patch('backend.src.app.app.jsonify', return_value=MagicMock())
